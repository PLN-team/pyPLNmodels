...
Y_{ij} \mid Z_{ij}, W_{ij} &\sim (1 - W_{ij}) \cdot \mathcal{P}(\exp(o_{ij} + Z_{ij})),
\end{align}
$$
...
Model parameters are given by $(B, \Sigma, \mu) = $(`coef`, `covariance`, `coef_clusters`).
...
Model parameters are given by $(B, \mu_k, \Sigma_k, \pi) = $(`coef`, `cluster_bias`, `covariances`, `weights`).
