...
$$
\begin{align}
Z_i &\sim \mathcal{N}(o_i + X_i^{\top} B, \Sigma), \\
...
- $\Sigma \in \mathcal{S}_{+}$ (`covariance`): covariance matrix of the latent variables $Z_i$
...
Y_{ij} \mid Z_{ij}, W_{ij} &\sim (1 - W_{ij}) \cdot \mathcal{P}(\exp(Z_{ij})),
\end{align}
$$
...
structural and sampling zeros. More details on modeling zero-inflation can be
...
$$
\begin{align}
Z_i &\sim \mathcal{N}(o_i + X_i^{\top} B, \Sigma), \quad \operatorname{rank}(\Sigma) = q \\
Y_{ij} \mid Z_{ij} &\sim \mathcal{P}(\exp(Z_{ij})).
\end{align}
$$

The rank $q$ must be chosen (e.g., via cross-validation). See @chiquet2018variational for details.
...
Constraints on $\Phi$ ensure that $\Sigma^\epsilon$ is positive definite.


## `PlnNetwork`: Sparse Dependency Structures
...
$$
\begin{align}
Z_i &\sim \mathcal{N}(o_i + X_i^{\top} B, \Sigma), \quad \|\Sigma^{-1}\|_1 \leq C \\
Y_{ij} \mid Z_{ij} &\sim \mathcal{P}(\exp(Z_{ij})).
\end{align}
$$
...
## `PlnLDA`: Latent Space Discriminant Analysis
...
## `PlnMixture`: Latent Class Modeling

The `PlnMixture` model captures subpopulation structure in the data by modeling
...
Z_i \mid c_i = k & \sim \mathcal{N}(\mu_k, \Sigma_k), \quad \text{for unknown cluster memberships } c_i,\\
Y_{ij} \mid Z_{ij} &\sim \mathcal{P}(\exp(Z_{ij})).
\end{align}
$$
