...
In this tutorial, we analyse zero-inflated count data using the
`pyPLNmodels` package.
...
In this tutorial, we analyze real count data from the microcosm
...
of the respective categorical variables.
 For visualization purposes, we consider only
...
Variables not explained by the model (called either 'exogenous' or 'covariates') are available:
...
We first fit a `Pln` model without zero-inflation. Check about the `Pln` model here ??
