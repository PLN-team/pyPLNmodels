...
may fail to capture important insights, such as correlations between variables.
...
rank increases the trade-off in parameter estimation accuracy.
...
* Retrieve the latent variables $Z$
...
For more details on formula syntax and model initialization, including handling offsets, refer to the package documentation.

Printing the model provides key information about its configuration:
...
The latent variables \(Z\), which capture the underlying structure of the data, are accessible via the `latent_variables` attribute:
...
**Note:** P-values are not available in the `PlnPCA` model.
...
While an optimal rank may not always be apparent, the best model can be accessed using the `.best_model()` method. The selection criterion can be `AIC`, `BIC`, or `ICL`:
