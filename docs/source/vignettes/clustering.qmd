...
dumb_lda.predict_clusters(endog_test, exog=torch.randn(endog_test.shape[0], 2))
```
...
One can access the individual models in the collection, using the cluster number as a key:
