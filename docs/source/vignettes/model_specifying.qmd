...
$$ Y_{ij}| Z_{ij} \sim \mathcal P(\exp(o_{ij} + Z_{ij})), \quad Z_{i}\sim \mathcal N(o_i + X_i^{\top} B, \Sigma),$$
with input data
...
which is accessible via the [NCBI BioProject](https://www.ncbi.nlm.nih.gov/bioproject/?term=PRJNA875059).
This dataset features microbiota members sampled from various body sites of dairy cows
throughout their lactation period. The data includes response variables (`endog`) and explanatory variables:
...
categorical variables. Depending on the analysis goals, you can select different
...
This line specifies a model with an intercept (1) and the categorical covariate site.
...
```
This corresponds to a model with latent structure but no fixed effects.
...
One can directly fit the model after specifying it:
