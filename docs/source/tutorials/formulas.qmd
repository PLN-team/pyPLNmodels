...
Constraints on $\Phi$ ensure that $\Sigma^\epsilon$ is positive definite. Model
parameters are given by $(B, \Sigma, \Phi) =$ (`coef`, `covariance`,
`ar_coef`).
...
$$
\begin{align}
Z_i &\sim \mathcal{N}(X_i^{\top} B, \Sigma), \quad \|\Sigma^{-1}\|_1 \leq \lambda \\
...
The hyperparameter $\lambda$ controls the sparsity level. A non-zero entry in
