...
class IMPS_PLN():
    ...
    def fit(self, Y, O, covariates, acc=0.005, N_epoch_max=500, lr=0.1,
            N_iter_max_mode=100, lr_mode=0.3, VR='SAGA', batch_size=40,
        ...
        Args :
               ...
               N_epoch_max: int, optional. The maximum number of times the algorithm will loop over
               ...
               lr: float greater than 0, optional. The learning rate of the batch gradient ascent.
                   Default is 0.1.
               N_iter_max_mode: int, optional. The maximum iteration you are ready to do to find the
                   mode of the posterior (i.e. the right importance law). Default is 100.
               lr_mode: float greater than 0, optional. The learning rate of the gradient ascent
                   finding the mode of the posterior. Default is 0.3.
...
class fastPLN():
    ...
    def fit(self, Y, O, covariates, N_iter_max=200, tol=1e-1,
...
def ELBO_PCA(Y, O, covariates, M, S, C, beta):
    ...
    second = torch.sum(-torch.exp(A + 1 / 2 *
...
class fastPLNPCA():
    ...
    def fit(self, Y, O, covariates, N_iter_max=5000, tol=1e-3, optimizer=torch.optim.Rprop,
    ...
    def show(self, name_doss='fastPLNPCA_graphic', save=False):
        ...
        ax[0].plot(abscisse[20:], - np.array(self.normalized_ELBOs)
                   [20:], label='Negative ELBO')
        ax[0].legend()
        ax[0].set_yscale('log')
        ax[0].set_title('Negative ELBO')
