import os 
from loss import ELBOnoPCA, ELBOPCA 
from closedForms import closedBeta, closedSigma
...
class PLN(): 
    ...
    def putParametersToDevice(self): 
        for parameter in self.parametersNeedingUpdateList: 
            parameter.requires_grad_(True)

    @property 
    @abstractmethod
    def parametersNeedingUpdateList(self): 
        pass
    @abstractmethod
    def basicInitVarParameters(self): 
        pass 

    def fit(self, Y, O, covariates, nbIterationMax=15000, lr=0.01, classOptimizer=torch.optim.Rprop,
            tol=1e-3, doGoodInit=True, verbose=False): 
        self.t0 = time.time()
        if self.fitted == False: 
            self.plotargs = PLNPlotArgs(self.window)
            self.initFromData(Y,O,covariates, doGoodInit)
        optim = classOptimizer(self.parametersNeedingUpdateList, lr = lr)
        nbIterationDone = 0  
        stopCondition = False 
    ...
    def printEndFitMessage(self, stopCondition, tol): 
        if stopCondition:
            print('Tolerance {} reached in {} iterations'.format(tol, self.plotargs.length))
        else:
            print('Maximum number of iterations reached : ',
                  self.plotargs.length, 'last delta = ', np.round(self.plotargs.deltas[-1],8))
    ...
    def computeCriterionAndUpdatePlotArgs(self, loss, tol):
        ...
        if self.plotargs.length > self.window :
            delta = abs(self.plotargs.normalizedELBOsList[-1] - self.plotargs.normalizedELBOsList[-1 - self.window])
...
class PLNnoPCA(PLN): 
    ...
    def computeELBO(self): 
        return ELBOnoPCA(self.Y, self.O, self.covariates, self.M, self.S, self.Sigma, self.beta)
    
    def updateClosedForms(self):
        self.beta = closedBeta(self.covariates, self.M) 
        self.Sigma = closedSigma(self.covariates, self.M, self.S, self.beta, self.n)
...
class PLNPCA(PLN): 
    ...
    def computeELBO(self): 
        return ELBOPCA(self.Y, self.O, self.covariates, self.M, self.S, self.C, self.beta)
