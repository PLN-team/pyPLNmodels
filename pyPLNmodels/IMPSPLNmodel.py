...
class IMPSPLN:
    ...
    def __init__(self, q, nbAverageParam=100, nbAverageLikelihood=8, propLaw='student', current = True):
    ...
    def initUsefulListsAndTensors(self):
        self.CriterionCounterList = [
            0
        ] * self.nbPlateauBeforeIncrementingCriterion  # counter for the criterion
        self.criterionList = [0]  # store the criterion to plot it after.
        self.timeToFindModeList = list() 
        self.timeToEstimGradList = list()
        self.runningTimes = list()  
        self.logLikelihoodList = list()  
        self.lastLikelihoods = list()  
        ...
        self.initLastParameters()
    def initLastParameters(self): 
    ...
    def initDataAndParams(self, Y, O, covariates, doGoodInit):
    ...
    def getBatch(self, batchSize):
        ...
        for i in range(nbFullBatch):
            yield self.returnBatch(indices, i * batchSize, (i + 1) * batchSize)
        # Last batch
        if lastBatchSize != 0:
            self.batchSize = lastBatchSize
            yield self.returnBatch(indices, -lastBatchSize, self.n)

    def returnBatch(self, indices, beginning, end):
        return (
                self.Y[indices[beginning:end]],
                self.covariates[beginning:end],
                self.O[indices[beginning:end]],
                indices[beginning:end],
            )
    ...
    def initFittingParameters(self, Y, O, covariates,lr, classOptimizer, doVarianceReduction, batchSize, nbMonteCarloSamples, criterionMax, nbPlateauBeforeIncrementingCriterion, doGoodInit, method): 
        self.passed = False
        self.listDiff = list()
        self.listMeanESS = list()
        self.nbIterMaxToFindMode = 700 
        self.t0 = time.time()
        self.criterionMax = criterionMax 
        self.nbPlateauBeforeIncrementingCriterion = nbPlateauBeforeIncrementingCriterion
        self.batchSize = batchSize 
        self.nbMonteCarloSamples = nbMonteCarloSamples
        if self.fitted == False: 
            self.initDataAndParams(Y,O,covariates, doGoodInit)
            self.initUsefulListsAndTensors()
        self.optimizer = classOptimizer([self.beta, self.C],lr = lr )
        if doVarianceReduction: 
            self.vr = SAGA([self.beta, self.C], self.n)
    ...
    def fit(
        ...
        nbEpochMax=500,
        lr=0.1,
        classOptimizer=torch.optim.Adagrad,
        doVarianceReduction=True,
        batchSize=40,
        nbMonteCarloSamples=200,
        criterionMax=15,
        nbPlateauBeforeIncrementingCriterion=5,
        doGoodInit=True,
        verbose=False,
        method='recycling'
    ):
        self.initFittingParameters(Y, O, covariates,lr,  classOptimizer, doVarianceReduction, batchSize, nbMonteCarloSamples, criterionMax, nbPlateauBeforeIncrementingCriterion, doGoodInit, method)
        for epochNumber in tqdm(range(nbEpochMax)):
            ...
            for YB, covariatesB, OB, selectedIndices in self.getBatch(batchSize):
                ...
                self.YB, self.covariatesB, self.OB = (
                    YB.to(device),
                    covariatesB.to(device),
                    OB.to(device),
                )
                self.selectedIndices = selectedIndices
                batchLogLike, batchGradC, batchGradBeta = self.getGradientsAndLogLike(
                )
                logLike += batchLogLike
                self.getStatWeights()
                self.updateParam(doVarianceReduction, batchGradBeta, batchGradC, selectedIndices)
    ...
    def updateParam(self, doVarianceReduction, batchGradBeta, batchGradC, selectedIndices): 
        if doVarianceReduction:
            self.vr.computeAndSetVarianceReducedGrad(
                [-batchGradBeta, -batchGradC], selectedIndices
            )
        else:
    ...
    def getGradientsAndLogLike(self):
        self.weights = self.computeGradientRequirementsAndGetWeights()
    ...
    def computeBestLoglike(
        self, nbMonteCarloSamples=1000,lrMode=0.001
    ):
        ...
        Args:
            ...
            self.nbIterMaxToFindMode : int, optional. The maximum number of iteration
        ...
        self.bestLogLike = self.inferBatchPTheta()
    ...
    def computeCriterion(self, verbose=True):
        ...
        triggered = int(
            self.CriterionCounterList[-1]
            - self.CriterionCounterList[-self.nbPlateauBeforeIncrementingCriterion - 1]
            > self.nbPlateauBeforeIncrementingCriterion - 1
        )
        self.criterionList.append(self.criterionList[-1] + triggered)
        if triggered > 0 and verbose:
            print(
                "Criterion updated : ", self.criterionList[-1], "/", self.criterionMax
            )
    ...
    def inferBatchPTheta(self):
        self.weights = self.computeGradientRequirementsAndGetWeights()
        perSampleLogLike = torch.log(torch.mean(self.weights, axis=0)) + self.const
        return torch.mean(perSampleLogLike)

    def computeGradientRequirementsAndGetWeights(self):
        ...
        right mean and variance, found by calling findMeanProposition and
        getBatchBestVar methods. The formula of the variance
        try: 
            #pass 
            self.meanProposition = self.expectationMean
        except: 
            pass 
    ...
    def getSamples(self):
        if self.propLaw == 'gaussian':
            self.samples = sampleGaussians(
                self.nbMonteCarloSamples, self.meanProposition, self.sqrtSigmaB
            )
        elif self.propLaw == 'student': 
            self.samples = sampleStudents(self.nbMonteCarloSamples, self.meanProposition, self.sqrtSigmaB,FREEDOMDEGREES)
    def getWeights(self):
        ...
        self.logG = logGaussianDensity(self.samples, self.meanProposition, self.SigmaB)
        self.logG = logStudentDensity(self.samples, self.meanProposition, self.SigmaB, 2)
    ...
    def getBatchBestVar(self):
        ...
        """
        batchMatrix = torch.matmul(self.C.unsqueeze(2), self.C.unsqueeze(1)).unsqueeze(
            0
        )
        CW = torch.matmul(self.C.unsqueeze(0), self.meanProposition.unsqueeze(2)).squeeze()
        common = (
    ...
    def getBatchGradLogPostBeta(self):
        ...
        """
        XY = torch.matmul(self.covariatesB.unsqueeze(2), self.YB.unsqueeze(1))
        XB = torch.matmul(
            self.covariatesB.unsqueeze(1), self.beta.unsqueeze(0)
        ).squeeze()
        CV = torch.matmul(
        ...
        Xexp = torch.matmul(
            self.covariatesB.unsqueeze(0).unsqueeze(3),
    ...
    def getGradBeta(self):
        ...
        return torch.sum(
            torch.multiply(
                gradLogPost, self.normalizedWeights.unsqueeze(2).unsqueeze(3)
            ),
            axis=0,
        )
    ...
    def getStatWeights(self):
        ...
        effectiveSampleSize = (
            torch.div(
                torch.sum(self.normalizedWeights, axis=0) ** 2,
                torch.sum(self.normalizedWeights ** 2, axis=0),
            )
            / self.nbMonteCarloSamples
        )
    ...
    def showLoss(self, ax=None, save=False, nameDoss="IMPSPLNLogLikelihood"):
        ...
        if ax is None:
            ...
            toShow = False
        ax.plot(self.runningTimes, -np.array(self.logLikelihoodList), label=self.NAME)
    ...
    def findMeanProposition(self): 
        if self.epochNumber <1 :
    ...
    def findMeanPropositionWithRecycling(self): 
        if self.epochNumber < 1: 
            raise ValueError('You need to do one epoch before recycling')
                # self.batchUnLogPosterior = batchUnLogPosterior
        if self.current: 
            self.getNewNormalizedWeights()
        self.expectationMean = torch.sum(
            torch.multiply(
                self.normalizedWeights.unsqueeze(2), self.samples
            ),
            axis=0,
        )
        self.meanProposition = torch.clone(self.expectationMean) 

    def getNewNormalizedWeights(self): 
        def batchUnLogPosterior(W):
                        return batchLogPWgivenY(
                            self.YB, self.OB, self.covariatesB, W, self.C, self.beta
                        )
        ## on recalcule log f avec les nouveaux beta et C 
        self.logF = batchUnLogPosterior(self.samples)
    ...
    def findMeanPropositionWithGradientAscent(self, eps=9e-9):
        ...
        Returns :
            None, but compute and stock the mode in self.meanProposition and the starting point.
        """
        ...
        i = 0
        stopCondition = False
        while i < self.nbIterMaxToFindMode and stopCondition == False:
            ...
            if i == 1 and self.epochNumber > 5:
                optim.state_dict()["state"][0]["step_size"] = (
                    5 * self.modeStepSizes[self.selectedIndices, :]
                )
            ...
            i += 1
        # Stock the mode
        self.meanProposition = torch.clone(W.detach())
        self.startingPoint[self.selectedIndices] = torch.clone(W.detach())
        self.modeStepSizes[self.selectedIndices, :] = optim.state_dict()["state"][0][
            "step_size"
        ]
    ...
    def plotRuntime(self):
        ...
        toPlot = {
            "Gradient estimation": lEstim,
            "Finding mode": lMode,
            "Total runtime": lTotal,
        }
